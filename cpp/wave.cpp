...
Wave::Wave(int N, int M, float L_x, float L_z, float omegax,float omegay, float V, float lambda) :
	N(N), M(M),
	omega_hat(normalize(vec2(omegax,omegay))),
	V(V), L_x(L_x), L_z(L_z),
	A(4e-7f),
	lambda(lambda)
{
...
void Wave::buildField(float time)
{
	...
	// Eq20 ikh_twiddle
	auto* slope_x_term = new complex<float>[kNum];
	auto* slope_z_term = new complex<float>[kNum];

	// Eq29
	auto* D_x_term = new complex<float>[kNum];
	auto* D_z_term = new complex<float>[kNum];
